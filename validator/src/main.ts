...
const main = async () => {
    ...
    const polka = await polkadot.newHelper(args.xnode, freezer_abi, args.freezer)
    const elrd = await elrond.newHelper(args.enode, Buffer.from(args.privk), args.sender)
